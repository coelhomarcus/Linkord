...
async function handleMe(request: FastifyRequest, reply: FastifyReply): Promise<void> {
  ...
  sendJson(reply, 200, { user: { id: sess.userId, username: sess.username, displayName: sess.displayName, avatar: sess.avatar, avatarColor: sess.avatarColor, role: sess.role } });
}

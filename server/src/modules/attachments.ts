...
async function fetchImageFromUrl(rawUrl: string, maxBytes: number, redirectsLeft = MAX_IMAGE_URL_REDIRECTS): Promise<FetchImageResult> {
  let parsed: URL;
  try {
 ...
 * server-side first (see fetchImageFromUrl) and then goes through the exact
 * same crop/encode path below. */
